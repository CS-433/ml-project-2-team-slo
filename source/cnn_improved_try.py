# -*- coding: utf-8 -*-
# -*- author : Fabio Palmisano -*-
# -*- date : 2023-12-04 -*-
# -*- Last revision: 2023-12-04 (Fabio Palmisano)-*-
...
class Improved_CNN(nn.Module):
    def __init__(self, patch_size=constants.WINDOW_SIZE):
        """Constructor."""
        ...
        self.dropout1 = nn.Dropout(0.5)  # Adjusted dropout rate
        ...
        self.dropout2 = nn.Dropout(0.5)  # Adjusted dropout rate
        self.relu4 = nn.LeakyReLU(0.1)

        self.fc1 = nn.Linear(64 * (patch_size // 8) * (patch_size // 8), 128)  # Increased hidden layer size
        self.fc2 = nn.Linear(128, 1)  # Added an additional fully connected layer

    def forward(self, x):
        """Forward pass."""
        x = self.pool1(self.relu1(self.bn1(self.conv1(x))))
        x = self.pool2(self.relu2(self.dropout1(self.bn2(self.conv2(x)))))
        x = self.pool3(self.relu3(self.bn3(self.conv3(x))))
        x = self.relu4(self.dropout2(self.bn4(self.conv4(x))))
        x = x.view(x.size(0), -1)
        x = self.fc1(x)
        x = self.fc2(x)
        return x
