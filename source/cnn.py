...
class CNN(nn.Module):
    ...
    def train_model(
        self, optimizer, scheduler, criterion, train_loader, val_loader, num_epochs=10
    ):
        """
        Train the model.
        ...
        for epoch in range(num_epochs):
            ...
            with torch.no_grad():
                ...
                f1 = f1_score(
                    torch.cat(tot_preds).cpu().numpy(),
                    torch.cat(tot_targets).cpu().numpy(),
                )
                print(
                    f"Epoch {epoch+1}/{num_epochs}, Loss: {test_loss}, Validation Accuracy: {accuracy:.4f}, F1 score: {f1:.4f}"
                )
