        ...
        if model_path is not None:
            if "test_set_images" not in os.listdir(data_path):
                ...
                raise FileNotFoundError(
                    f"Path {data_path} does not contain the test folder."
                )
        else:
            if "test_set_images" not in os.listdir(
                data_path
            ) and "training" not in os.listdir(data_path):
                # If the path does not contain the test and training folders, raise an error
                raise FileNotFoundError(
                    f"Path {data_path} does not contain the test and training folders."
                )
...
def run(data_path, csv_path, mask_path, model_path):
    ...
    torch.manual_seed(0)
    # Handle the paths
    data_path, csv_path, mask_path, model_path = handle_path(
        data_path, csv_path, mask_path, model_path
    )
    ...
    if model_path is None:
        ...
        patch_size = 128
        standardize = True
        myDatas = AdvancedProcessing(
            standardize=standardize, aug_patch_size=patch_size, blur=True
        )
    ...
    images_filenames = save_pred_as_png(
        preds, len(myTestDatas.imgs), folder_path=mask_path
    )
